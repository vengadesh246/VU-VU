...
function ShippingPage({ cart, onBack, onProceedToPayment, customerId, customerName }) {
          ...
          fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${latitude}&lon=${longitude}&zoom=18&addressdetails=1`)
            ...
            .then(data => {
              if (data && data.display_name) {
                ...
                // Auto-validate after location detection
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validateForm()) {
      // Save address to saved addresses if not already saved
      ...
      <div className="shipping-container">
                ...
                >
                  <div className="address-card-header">
                    ...
                    <div className="address-actions">
                      <button onClick={() => handleEditAddress(address.id)} className="edit-address-btn">
                        ✏️
                      </button>
                      <button onClick={() => handleDeleteAddress(address.id)} className="delete-address-btn">
                        🗑️
                      </button>
                    </div>
                  </div>
                  ...
                  {selectedAddressId === address.id && (
                    <div className="address-selected-badge">✅ Selected</div>
                  )}
                </div>
              ))}
            </div>
        ...
        {(showAddressForm || savedAddresses.length === 0) && (
          <div className="address-form-container">
            <h3>{isEditing ? '✏️ Edit Address' : savedAddresses.length > 0 ? '📝 Add New Address' : '📝 Enter Shipping Address'}</h3>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="location-actions">
                <button 
                  ...
                  className={`detect-location-btn ${isLocationDetected ? 'detected' : ''}`}
                  disabled={isLoading}
                >
              ...
              <div className="form-actions">
                {isEditing ? (
                  <>
                    <button type="button" onClick={handleUpdateAddress} className="proceed-btn">
                      💾 Update Address
                    </button>
                    <button 
                      type="button" 
                  ...
                  <button type="submit" className="proceed-btn">
                    {savedAddresses.length > 0 ? '✅ Use This Address' : 'Proceed to Payment →'}
                  </button>
                )}
              </div>
            </form>
        ...
        {!showAddressForm && savedAddresses.length > 0 && !isEditing && (
          <div className="form-actions">
            <button type="button" onClick={handleAddNewAddress} className="proceed-btn">
              ➕ Add New Address
            </button>
          </div>
        )}
